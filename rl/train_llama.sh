...
# Llama-3.2-11B-Vision-Instruct visual-stack GRPO on one 8-GPU node.
# This recipe requires the optional verl compatibility patch documented in README.md.
: "${VERL_ROOT:?Set VERL_ROOT to the patched verl v0.4.1 checkout}"
: "${MODEL_PATH:?Set MODEL_PATH to Llama-3.2-11B-Vision-Instruct}"
...
RUN_NAME=${RUN_NAME:-llama3_2_11b_vision_visionfoundry_grpo}
TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE:-32}
VISUAL_LR=${VISUAL_LR:-1e-6}
ADAPTER_LR=${ADAPTER_LR:-1e-6}
LLM_LR=${LLM_LR:-0}
...
export VERL_TORCH_DIST_TIMEOUT_SEC=${VERL_TORCH_DIST_TIMEOUT_SEC:-7200}
...
python3 -m verl.trainer.main_ppo \
  ...
  actor_rollout_ref.actor.optim.lr="${VISUAL_LR}" \
  actor_rollout_ref.actor.optim.enable_vl_lr_groups=True \
  actor_rollout_ref.actor.optim.visual_lr="${VISUAL_LR}" \
  actor_rollout_ref.actor.optim.adapter_lr="${ADAPTER_LR}" \
  actor_rollout_ref.actor.optim.llm_lr="${LLM_LR}" \
  +actor_rollout_ref.actor.optim.visual_lr_keywords="['vision_model']" \
  +actor_rollout_ref.actor.optim.adapter_lr_keywords="['multi_modal_projector']" \
